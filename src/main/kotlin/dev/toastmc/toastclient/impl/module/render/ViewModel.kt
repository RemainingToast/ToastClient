...
object ViewModel : Module("ViewModel", Category.RENDER) {
    ...
    var scaleX = number("SX", 0.75, -1, 2)
    var scaleY = number("SY", 0.60, -1, 2)
    var scaleZ = number("SZ", 1, -1, 2)
    var scaleGroup = group("Scale", scaleX, scaleY, scaleZ)

    var posX = number("PX", 0, -3, 3)
    var posY = number("PY", 0, -3, 3)
    var posZ = number("PZ", 0, -3, 3)
    var posGroup = group("Position", posX, posY, posZ)

    var rotationX = number("RX", 0, -180, 180)
    var rotationY = number("RY", 0, -180, 180)
    var rotationZ = number("RZ", 0, -180, 180)
