package dev.toastmc.client.gui.clickgui;
...
import dev.toastmc.client.ToastClient;
import dev.toastmc.client.gui.clickgui.components.Description;
import dev.toastmc.client.gui.clickgui.components.ModeSetting;
import dev.toastmc.client.gui.clickgui.components.SliderSetting;
import dev.toastmc.client.gui.clickgui.components.ToggleSetting;
...
import static dev.toastmc.client.gui.clickgui.ClickGuiScreen.keybindPressedCategory;
...
public class CategoryRenderer {
    ...
    public CategoryRenderer(int mouseX, int mouseY, Module.Category category, boolean clickedL, boolean clickedR) {
        ...
        if (TwoDRenderUtils.isMouseOverRect(getMouseX(), getMouseY(), getX(), getY(), getBoxWidth(), getBoxHeight())) {
            if (isClickedR()) {
        ...
        TwoDRenderUtils.drawTextBox(getXint(), getYint(), getBoxWidth(), getBoxHeight(), colors.categoryBoxColor, colors.categoryTextColor, colors.categoryPrefixColor, catBgColor, colors.categoryPrefix, category.toString());
        if (settings.getPositions(category.toString()).isExpanded()) {
            int u = 1;
            for (Module module : ToastClient.MODULE_MANAGER.getModulesInCategory(category)) {
                int moduleTextColor;
                int moduleBgColor;
                ...
                if (TwoDRenderUtils.isMouseOverRect(getMouseX(), getMouseY(), getX(), getYIteration(u), getBoxWidth(), getBoxHeight())) {
                    if (isClickedL()) {
                ...
                TwoDRenderUtils.drawTextBox(getXint(), (int) Math.round(getYIteration(u)), getBoxWidth(), getBoxHeight(), colors.moduleBoxColor, moduleTextColor, colors.modulePrefixColor, moduleBgColor, colors.modulePrefix, module.getName());
                u++;
                if (!module.getSettings().getSettings().isEmpty()) {
                    for (String modName : settings.getPositions(categoryString).getExpandedModules()) {
                        if (modName.equals(module.getName())) {
                            ...
                            TwoDRenderUtils.drawTextBox(getXint(), (int) Math.round(getYIteration(u)), getBoxWidth(), getBoxHeight(), colors.settingBoxColor, colors.settingOnTextColor, colors.settingPrefixColor, keybindBgColor, colors.settingPrefix, keybindText);
                            u++;
                        }
                    }
                }
    ...
    public boolean isMouseOverCat() {
        return TwoDRenderUtils.isMouseOverRect(getMouseX(), getMouseY(), getX() - 4, getY() - 4, getBoxWidth() + 4, getBoxHeight() + 2);
    }
        ...
        if (this.isMouseOverCat() && isCategory) {
            ...
            if (sliders.size() > 0) {
                for (Slider slider : sliders) {
                    if (TwoDRenderUtils.isMouseOverRect(mouseX, mouseY, slider.sliderKnobX - 4, slider.sliderPosY - 4, 10, 10)) {
