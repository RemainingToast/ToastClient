...
public class ClickGuiScreen extends Screen {
        ...
        if (descriptions) {
            for (Map.Entry<Module.Category, CategoryRenderer> categoryRendererEntry : categoryRenderers.entrySet()) {
                ...
                if (categoryRenderer.description != null) {
                    TwoDRenderUtils.drawTextBox(categoryRenderer.description.descPosX, categoryRenderer.description.descPosY, textRenderer.getWidth(settings.colors.descriptionPrefix + categoryRenderer.description.desc) + 4, height, settings.colors.descriptionBoxColor, settings.colors.descriptionTextColor, settings.colors.categoryPrefixColor, settings.colors.descriptionBgColor, settings.colors.descriptionPrefix, categoryRenderer.description.desc);
                    break;
                }
            }
        }
    ...
    public void onClose() {
        ...
        settings.saveColors();
        Objects.requireNonNull(ToastClient.MODULE_MANAGER.getModule(ClickGui.class)).disable();
    }

    @Override
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        if (keyCode != GLFW.GLFW_KEY_UNKNOWN) {
            if (keyCode == GLFW.GLFW_KEY_ESCAPE) ToastClient.clickGui.onClose();
            if (keyCode == GLFW.GLFW_KEY_RIGHT_SHIFT) ToastClient.clickGui.onClose();
            else if (keybindPressedCategory != null) keybindPressedCategory.setKeyPressed(keyCode);
            if (keyCode == Objects.requireNonNull(ToastClient.MODULE_MANAGER.getModule(ClickGui.class)).getKey()) ToastClient.clickGui.onClose();
        }
        return false;
    }
