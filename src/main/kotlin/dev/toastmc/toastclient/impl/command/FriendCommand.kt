...
object FriendCommand : Command("friend") {
                    ...
                    if (FriendManager.friends.isEmpty()) {
                        ...
                        FriendManager.friends.forEach {
                            if (text == null) {
                                text = lit("${Formatting.GREEN}${it.name}")
                                    .setStyle(
                                        Style.EMPTY
                                            .withHoverEvent(
                                                HoverEvent(
                                                    HoverEvent.Action.SHOW_TEXT, lit(
                                ...
                                val friend: Text =
                                    lit("${Formatting.DARK_GRAY}, ${Formatting.GREEN}${it.name}")
                                        .setStyle(
                                            Style.EMPTY
                                                .withHoverEvent(
                                                    HoverEvent(
                ...
                then(
                    createFriendArgument(
                        ...
                        { entry: ToastPlayer, _: CommandSource ->
                            ...
                            message(
                                lit("$prefix ${entry.name} has been ${Formatting.GREEN}added${Formatting.GRAY} as friend").formatted(
                                    Formatting.GRAY
                                )
                            )
                            0
                ...
                then(
                    removeFriendArgument(
                        ...
                        { entry: ToastPlayer, _: CommandSource ->
                            ...
                            message(
                                lit("$prefix ${entry.name} has been ${Formatting.RED}removed${Formatting.GRAY} as friend").formatted(
                                    Formatting.GRAY
                                )
                            )
                            0
    ...
    private fun createFriendArgument(
        fail: Function<ToastPlayer, CommandSyntaxException?>,
        ...
        return RequiredArgumentBuilder.argument<CommandSource, EntitySelector>(
            ...
            .executes { ctx ->
                ...
                val optionalPlayer =
                    mc.networkHandler!!.playerList.stream()
                        .filter { playerListEntry: PlayerListEntry ->
                            ...
                            if (playerName != null) {
                                return@filter playerListEntry.profile.name.equals(
                                    playerName,
                                    ignoreCase = true
                                )
                            } else {
                ...
                if (optionalPlayer.isPresent) {
                    ...
                    return@executes function.apply(
                        ToastPlayer(
                            entry.profile.name,
                            entry.profile.id
                        ), ctx.source as CommandSource
                    )
    ...
    private fun removeFriendArgument(
        fail: Function<ToastPlayer, CommandSyntaxException?>,
        function: BiFunction<ToastPlayer, CommandSource, Int>,
    ): RequiredArgumentBuilder<CommandSource, ToastPlayer> {
        return RequiredArgumentBuilder.argument<CommandSource, ToastPlayer>(
            "friend",
            FriendArgumentType.friend()
        )
            .executes { ctx ->
                val selector: ToastPlayer = "friend" from ctx
                val optionalFriend = FriendManager.friends.stream()
                    .filter {
                        return@filter it.name.equals(
                            selector.name,
                            true
                        ) || it.uuid == selector.uuid
                    }
                    .findAny()
