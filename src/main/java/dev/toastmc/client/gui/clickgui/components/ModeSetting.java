...
public class ModeSetting {
    public static void render(Module module, String settingName, ClickGuiSettings.Colors colors, double x, double mouseX, double mouseY, int curIt, boolean clickedL) {
        ...
        if (TwoDRenderUtils.isMouseOverRect(mouseX, mouseY, x, CategoryRenderer.getYIteration(curIt), CategoryRenderer.getBoxWidth(), CategoryRenderer.getBoxHeight())) {
            if (clickedL) {
        ...
        TwoDRenderUtils.drawTextBox((int) Math.round(x), (int) Math.round(CategoryRenderer.getYIteration(curIt)), CategoryRenderer.getBoxWidth(), CategoryRenderer.getBoxHeight(), colors.settingBoxColor, colors.settingOnTextColor, colors.settingPrefixColor, settingBgColor, colors.settingPrefix, settingName + ": " + module.getSettings().getMode(settingName));
    }
}
